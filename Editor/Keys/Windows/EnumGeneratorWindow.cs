...
namespace VaporEditor.Keys
{
    public class EnumGeneratorWindow : EditorWindow
    {
        ...
        private readonly List<BackingContext> _filteredEnums = new();
        ...
        public void CreateGUI()
        {
            ...
            var currentEnumGUIDs = AssetDatabase.FindAssets("t:TextAsset", new[] { ConfigPath });
        ...
        private void OnCreate()
        {
            ...
            foreach (var enumName in _enums)
            {
                if (customOrder)
                {
                    kvp.Add(new KeyGenerator.KeyValuePair(enumName.Value, currentValue, string.Empty));
                    switch (orderDirection)
                    {
                        case OrderDirection.CountUp:
                            currentValue++;
