...
rem Builds the VaporRpc Roslyn source generator and drops the DLL where Unity picks it up.
...
set "ROOT=%~dp0"
set "PROJ=%ROOT%src\VaporRpcGenerator.csproj"
set "DEST=%ROOT%..\..\Runtime\Network Objects\Analyzers"

where dotnet >nul 2>nul
if errorlevel 1 goto no_dotnet

echo ============================================================
echo  Building Vapor.Rpc.SourceGenerator  [%CONFIG%]
echo ============================================================

dotnet build "%PROJ%" -c %CONFIG% -v minimal
if errorlevel 1 goto build_failed

set "DLL=%ROOT%src\bin\%CONFIG%\netstandard2.0\Vapor.Rpc.SourceGenerator.dll"
if not exist "%DLL%" goto no_dll

if not exist "%DEST%" mkdir "%DEST%"
copy /y "%DLL%" "%DEST%\Vapor.Rpc.SourceGenerator.dll" >nul
if errorlevel 1 goto copy_failed

echo.
echo [DONE] Generator copied to:
echo   %DEST%\Vapor.Rpc.SourceGenerator.dll
echo.
echo Switch to Unity and let it reimport. Check Console for "VRPC" diagnostics.
goto end

:no_dll
echo [ERROR] Build reported success but the DLL was not found at:
echo   %DLL%
echo Check TargetFramework in src\VaporRpcGenerator.csproj.
goto end
